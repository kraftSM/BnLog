...
# BnLog
## Рабочая версия  итогового проекта C# of SkillFactory.(V1.4.00 от 2023-12-11 HW4  branch)
 - Рабочая версия  итогового проекта C# of SkillFactory.(V1.4.00 от 2023-12-11 HW4  branch)
 - Реализация  требований HW4 (HW4  branch)
 - Реализация  API, надстроенного над уровнем BnLog.VAL (BnLog.VAL и ниже является общей кодовой базой для BnLog.APL и BnLog.API)
 + - в BnLog.API загружает собственно 2-й вариант приложения (API) в котором реализован отдельный набор контролллеров
 + - - Реализация   OpenDoc + Swagger(UI). API тестировалось в нем + PostMan
 + + - Реализация   NLog (в контроллеры вставлен, но сами Log-записи не создаются. конфигурация NLog скопирована из  BnLog.APL  )
 - Реализация разграничения доступа на основе фильтра  [Authorize] / [Authorize(Roles = "Admin, User, Moderator, Designer")] и т.п. не получилась
 -  - В чем дело разобраться так и не удалось, поэтому все Action доступны полностью (где-то мог просмотреть и не разкомментить этот фильтр, и тут возможно ошибка)
## Версия  итогового проекта C# of SkillFactory.(V1.3.1 от 2023-11-29 HW3  branch)
...
## Версия  итогового проекта C# of SkillFactory.(V1.3.02 от 2023-11-29)
  - - Реализация  требований HW3 (HW3  branch)
  - + + TODO
...
## Bерсия  итогового проекта C# of SkillFactory.(V1.2.2 от 2023-11-16)
